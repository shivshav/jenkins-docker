...
LDAP_NAME=${LDAP_NAME:-${10}}
LDAP_VOLUME=${LDAP_VOLUME:-${11}}
SLAPD_DOMAIN=${SLAPD_DOMAIN:-${12}}
NEXUS_REPO=${NEXUS_REPO:-${13}}
CHECKOUT_DIR=./git
...
#sed -i "s/{LDAP_ACCOUNTBASE}/${LDAP_ACCOUNTBASE}/g" ${BASEDIR}/${DEFAULT_CONFIG_XML}
...
###################################################################################################
###### If Jenkins 'user' exists Then pull user's password out of ENV var and then add Jenkins SSH to gerrit
###### Else Skip all SSH config stuff and Gerrit stuff
###################################################################################################
